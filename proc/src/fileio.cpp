...
bl_fgetsBuffered(void *space, fileiterator_t *fi, char **str) {
  ...
  line = (char *) ALLOCMEMORY(space, NULL, char, linebuffersize);

  //if the iterator is called for the first time with buffers
  if(!fi->buffer) {
    fi->buffer = (char *) ALLOCMEMORY(space, NULL, char, fi->buffersize+1);
    fi->ptr = 0;
    fi->bufferfill =0;
 ...
 // fprintf(stderr, "prior to loop current char is '%c'\n", fi->buffer[fi->ptr]);
  ...
  while(fi->ptr < fi->bufferfill && fi->buffer[fi->ptr] != '\n') {
    ...
    if(len+1 == linebuffersize) {
      ...
      line = (char *) ALLOCMEMORY(space, line, char, linebuffersize+1);
...
initFileIterator (void *space, char *filename)
{
  ...
  fileiterator_t *fi;
  fi = (fileiterator_t *) ALLOCMEMORY(space, NULL, fileiterator_t, 1);
...
readcsvlines(void *space,
  ...
  csv = (stringset_t **) ALLOCMEMORY(space, NULL, stringset_t*, linecount);
  ...
  while(i < linecount && (len=bl_fgetsBuffered(space, fi, &line)) != EOF) {
    ...
    csv[i] = tokensToStringset(space, p, line, len);
