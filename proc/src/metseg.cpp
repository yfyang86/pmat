...
/**
 * Init the segment_t structure.
 * @param seg a segment_t structure (pointer): See also `segment_t`.
 * @return NULL
 */
...
/**
 * destruct the segment_t structure (pointer).
 * @param seg a segment_t structure: See also `segment_t`.
 * @return NULL
 */
void destructSegment(segment_t *seg) {
...
/**
 * destruct the cpg structure (pointer).
 * @param seg a cpg structure: See also `cpg`.
 * @return NULL
 */
void destructCpg(cpg_t *cpg) {
...
/**
 * destruct the cpg segmentset_t (pointer).
 * @param set a cpg segmentset_t: See also `segmentset_t`.
 * @return NULL
 */
void destructSegmentSet(segmentset_t *set) {
...
/**
 * Init the segmentset_t structure.
 * @param seg a segmentset_t structure (pointer): See also `segmentset_t`.
 * @return NULL
 */
void initSegmentSet(segmentset_t *set) {
  set->n=0;
  set->firststop=-1;
  set->nextchr=NULL;
  set->nextstart=-1;
...
/**
 * Add a segmentset_t (`segment_t`) element to segmentset_t
 * @param set a segmentset_t structure (pointer): See also `segmentset_t`, `segment_t`, and `initSegmentSet`.
 * @return The added segment_t pointer
 */
segment_t * addNewSegmentToSet(segmentset_t *set) {
  segment_t *seg = NULL;
  seg = (segment_t *) ALLOCMEMORY(NULL, NULL, segment_t, 1);
  initSegment(seg);
...
/**
 * Get the segmentset_t (`segment_t`), index number is `n`
 * @param set a segmentset_t structure (pointer): See also `segmentset_t`, `segment_t`, and `initSegmentSet`.
 * @param n, an integer: index number
 * @return segment_t pointer. n > the size of `set`, return a `NULL` pointer.
 */
segment_t * getSegment(segmentset_t *set, int n) {
    segment_t *s = NULL;
...
/**
 * Remove a given element from `segmentset_t`
 * @param set a segmentset_t structure (pointer): See also `segmentset_t`, `segment_t`, and `initSegmentSet`.
 * @param seg, an segment_t pointer: segment_t element
 * @return no return: `set` is changed.
 */
void removeThisSegmentFromSet(segmentset_t *set, segment_t *seg) {
    if(seg->parent == NULL) {
        if(set->n==1) {
            set->head=NULL;
            set->tail=NULL;
...
void removeSegmentFromSet(segmentset_t *set, int n) {
    //no object to remove
...
void setSegment(segment_t *seg, char *chr, int start, int stop,
...
double get_meandiff(cpg_t *cpg,double *a, int m, double *b, int n) {
double meandiff = 0.;
double mean1 = 0.;
double mean2 = 0.;

for(size_t i=0; i<m; i++) { mean1+=a[i]; }
mean1/=(double) m;

for(size_t i=0; i<n; i++) { mean2+=b[i]; }
mean2/=(double) n;
...
cpg->methB=mean2;
  ...
  for(size_t i=0; i<n; i++){ meandiff += fabs(a[i] - b[i]); }
  meandiff = meandiff/(double)n;
#endif
...
counter(double x, double y, double *x0, double *x1, int m,
  ...
  c = (double **) ALLOCMEMORY(NULL, NULL, double*, 2);
  l = (double **) ALLOCMEMORY(NULL, NULL, double*, 2);

  c[0] = (double *) ALLOCMEMORY(NULL, NULL, double, 4);
  c[1] = (double *) ALLOCMEMORY(NULL, NULL, double, 4);
  l[0] = (double *) ALLOCMEMORY(NULL, NULL, double, 5);
  l[1] = (double *) ALLOCMEMORY(NULL, NULL, double, 5);
...
void kstest(segment_t *seg , int a, int b, char mindiff, char mincpgs, char test,
  ...
  la = (double **) ALLOCMEMORY(NULL, NULL, double*, 2);
  lb = (double **) ALLOCMEMORY(NULL, NULL, double*, 2);

  la[0] = (double *) ALLOCMEMORY(NULL, NULL, double, l*noA);
  la[1] = (double *) ALLOCMEMORY(NULL, NULL, double, l*noA);
  lb[0] = (double *) ALLOCMEMORY(NULL, NULL, double, l*noB);
  lb[1] = (double *) ALLOCMEMORY(NULL, NULL, double, l*noB);
  ...
  x = (double *) ALLOCMEMORY(NULL, NULL, double, l);
  y = (double *) ALLOCMEMORY(NULL, NULL, double, l);
  ...
  #ifdef __FOLDED_NORMAL__
    for(size_t i=0; i<l*noA; i++){ meandiff += fabs(la[0][i]-lb[0][i]); }
      meandiff = meandiff/(double)(l*noA);
  #endif
 ...
 *
 */
void means(segment_t *seg , int a, int b, int *grpA, int noA, int *grpB, int noB, double *smeans){
...
calcSingleDiffSum(segment_t *s, int *grpA, int noA, int *grpB, int noB){
  ...
  S = (double **) ALLOCMEMORY(NULL, NULL, double*, s->n);

  for(i=0; i < s->n; i++) {
    S[i] = (double *) ALLOCMEMORY(NULL, NULL, double, 3);
    ...
    double *ks1, double *ks2, double *ks3, double *KS, segment_t *max)
{
    ...
    if(max) {
      segment_t *copy = (segment_t *) ALLOCMEMORY(NULL, NULL, segment_t, 1);
  ...
  while(!bl_segmentstackIsEmpty(&stack) || a != -1) {
    ...
    if(a != -1 && child <= 2) {
      if(ab[0] == -1) {
        ...
        dimZ = b - a + 1;
        XZ = (double **) ALLOCMEMORY(NULL, NULL, double*, dimZ);

        for(i=0; i < dimZ; i++) {
          XZ[i] = (double *) ALLOCMEMORY(NULL, NULL, double, dimZ);
        ...
        if(child == 0) {
          ...
          if(ab[0] > 0 && n <= m) {
            if(m-n >= nfo->mincpgs) {
              ...
              breaks = (segment_t *) ALLOCMEMORY(NULL, breaks, segment_t, (*nbreaks)+1);
...
segmenterSTK(segment_t *seg, segment_t *globalbreaks, int *nglobal, double **XS,
  ...
  while(!bl_segmentstackIsEmpty(&stack) || a != -1) {
    ...
    if(a != -1) {
      ...
      globalbreaks = (segment_t *) ALLOCMEMORY(NULL, globalbreaks, segment_t, (*nglobal)+1);
...
output(segment_t *seg, segment_t *breaks, int nglobal, double **XS,
  ...
  if(nfo->outputList->i >= nfo->outputList->n) {
        ...
        nfo->outputList->segment_out_st = (segment_out *) ALLOCMEMORY(NULL, nfo->outputList->segment_out_st, segment_out, nfo->outputList->n);
  }
  ...
  for(i=0; i<nglobal; i++) {
    ...
    if(b->prob > 1) {
      if(tmp == NULL) {
        ...
        tmp = (segment_t *) ALLOCMEMORY(NULL, NULL, segment_t, 1);
        tmp->chr = b->chr;
        tmp->start=b->start;
        tmp->stop=b->stop;
        tmp->prob=b->prob;
 ...
 // ks[2]=p;
      ...
      if(tmp != NULL) {
        ...
        if(ks[0]<2) {
            nfo->outputList->segment_out_st[nfo->outputList->i].chr = (char *) ALLOCMEMORY(NULL, NULL, char, strlen(seg->chr)+1);
            nfo->outputList->segment_out_st[nfo->outputList->i].chr = strcpy(nfo->outputList->segment_out_st[nfo->outputList->i].chr,seg->chr);
            nfo->outputList->segment_out_st[nfo->outputList->i].start = seg->pos[tmp->start]-1;
            nfo->outputList->segment_out_st[nfo->outputList->i].stop = seg->pos[tmp->stop];
            nfo->outputList->segment_out_st[nfo->outputList->i].p = ks[0];
            nfo->outputList->segment_out_st[nfo->outputList->i].q = -1;
            nfo->outputList->segment_out_st[nfo->outputList->i].meandiff = ks[1];
            nfo->outputList->segment_out_st[nfo->outputList->i].mwu = ks[2];
            nfo->outputList->segment_out_st[nfo->outputList->i].length = (tmp->stop-tmp->start+1);

            double me[] = {-2,-2};
            means(seg,tmp->start,tmp->stop, grpA, noA, grpB, noB, me);
            nfo->outputList->segment_out_st[nfo->outputList->i].methA = me[0];
            nfo->outputList->segment_out_st[nfo->outputList->i].methB = me[1];
       ...
       nfo->outputList->segment_out_st[nfo->outputList->i].chr = (char *) ALLOCMEMORY(NULL, NULL, char, strlen(seg->chr)+1);
        nfo->outputList->segment_out_st[nfo->outputList->i].chr = strcpy(nfo->outputList->segment_out_st[nfo->outputList->i].chr,seg->chr);
        nfo->outputList->segment_out_st[nfo->outputList->i].start = seg->pos[b->start]-1;
        nfo->outputList->segment_out_st[nfo->outputList->i].stop = seg->pos[b->stop];
        nfo->outputList->segment_out_st[nfo->outputList->i].p = b->prob;
        nfo->outputList->segment_out_st[nfo->outputList->i].q = -1;
        nfo->outputList->segment_out_st[nfo->outputList->i].meandiff = b->meandiff;
        nfo->outputList->segment_out_st[nfo->outputList->i].mwu = b->test;
        nfo->outputList->segment_out_st[nfo->outputList->i].length = (b->stop-b->start+1);
...
/*
  ...
  */
    ...
    nfo->outputList->segment_out_st[nfo->outputList->i].chr = (char *) ALLOCMEMORY(NULL, NULL, char, strlen(seg->chr)+1);
    nfo->outputList->segment_out_st[nfo->outputList->i].chr = strcpy(nfo->outputList->segment_out_st[nfo->outputList->i].chr,seg->chr);
    nfo->outputList->segment_out_st[nfo->outputList->i].start = seg->start-1;
    nfo->outputList->segment_out_st[nfo->outputList->i].stop = seg->stop;
    nfo->outputList->segment_out_st[nfo->outputList->i].p = ks[0];
    nfo->outputList->segment_out_st[nfo->outputList->i].meandiff = ks[1];
    nfo->outputList->segment_out_st[nfo->outputList->i].mwu = ks[2];
    nfo->outputList->segment_out_st[nfo->outputList->i].length = seg->n;
    nfo->outputList->segment_out_st[nfo->outputList->i].methA = me[0];
    nfo->outputList->segment_out_st[nfo->outputList->i].methB = me[1];
...
//cpg->chr, cpg->start,cpg->stop,ratio,p);
    ...
    nfo->outputList->segment_out_st[nfo->outputList->i].chr = (char *) ALLOCMEMORY(NULL, NULL, char, strlen(chr)+1);
    nfo->outputList->segment_out_st[nfo->outputList->i].chr = strcpy(nfo->outputList->segment_out_st[nfo->outputList->i].chr,chr);
    nfo->outputList->segment_out_st[nfo->outputList->i].start = start-1;
    nfo->outputList->segment_out_st[nfo->outputList->i].stop = stop;
    //nfo->outputList->segment_out_st[nfo->outputList->i].p = p;
    nfo->outputList->segment_out_st[nfo->outputList->i].meandiff = ratio;
    nfo->outputList->segment_out_st[nfo->outputList->i].mwu = p;
    nfo->outputList->segment_out_st[nfo->outputList->i].length = 1;
    nfo->outputList->segment_out_st[nfo->outputList->i].methA = methA;
    nfo->outputList->segment_out_st[nfo->outputList->i].methB = methB;
...
fillNAN(double *values, int *grpA, int noA, int *grpB, int noB, metseg_t *nfo) {
  //    fprintf(stderr,"#Ueberhaupt schaetzen\n");
    int na=0;
    int nb=0;
    double *groupA = (double *) ALLOCMEMORY(NULL, NULL, double, noA);
    double *groupB = (double *) ALLOCMEMORY(NULL, NULL, double, noB);
    double varA;
    double varB;
    double meanA = 0.0;
    double meanB = 0.0;
...
// Options
  manopt_initoptionset(&optset, argv[0], NULL,
      (char *)"metilene - a tool for fast and sensitive detection of differential DNA methylation\n\nDataInputFile\t\tneeds to be SORTED for chromosomes and genomic positions",
      (char *)"Implemented by Frank Juehling and Steve Hoffmann\n  2015-2016 Bioinformatik Leipzig\n",
      version,
      (char *)"Please report bugs to [frank,steve]@bioinf.uni-leipzig.de");
  ...
  for(k=2; k < csv[0]->noofstrings; k++) {
    if(strncmp(nfo.nameA, csv[0]->strings[k].str, strlen(nfo.nameA)) == 0) {
      grpA = (int *) ALLOCMEMORY(NULL, grpA, int, noA+1);
      grpA[noA] = k-2;
      noA++;
    } else
      if (strncmp(nfo.nameB, csv[0]->strings[k].str, strlen(nfo.nameB)) == 0) {
        grpB = (int *) ALLOCMEMORY(NULL, grpB, int, noB+1);
        grpB[noB] = k-2;
        noB++;
      }
  }
  ...
  idle = nfo.threads;
  schedule = (volatile char *)ALLOCMEMORY(NULL, NULL, volatile char, nfo.threads);
  for(i=0; i < nfo.threads; i++) {
    schedule[i]=0;
  }

  threads = (pthread_t *) ALLOCMEMORY(space, NULL, pthread_t, nfo.threads);
  th_nfo = (metseg_t *) ALLOCMEMORY(space, NULL, metseg_t, nfo.threads);
...
//###################### SINGLE CpG mode #########################
  if(nfo.mode == 3) {
      ...
      nfo.outputList = (list_out *) ALLOCMEMORY(NULL, NULL, list_out, 1);
      nfo.outputList->segment_out_st = (segment_out *) ALLOCMEMORY(NULL, NULL, segment_out, 1);
      nfo.outputList->n=1000000;
      nfo.outputList->i=0;
      nfo.outputList->numberTests=0;
      nfo.outputList->segment_out_st = (segment_out *) ALLOCMEMORY(NULL, NULL, segment_out, nfo.outputList->n);
...
//check missing numbers
            double *values = (double *)ALLOCMEMORY(NULL, NULL, double, csv[0]->noofstrings);
         ...
         //       fprintf(stderr,"...done\n");
            ...
            cpg_t *cpg = (cpg_t *) ALLOCMEMORY(NULL, NULL, cpg_t, 1);
            double *groupA = (double *) ALLOCMEMORY(NULL, NULL, double, noA);
            double *groupB = (double *) ALLOCMEMORY(NULL, NULL, double, noB);
...
//                  groupB[i] = atof(csv[0]->strings[grpB[i]+2].str);
            ...
            cpg->groupA=groupA;
            cpg->groupB=groupB;
            cpg->chr = (char *)ALLOCMEMORY(NULL, NULL, char, csv[0]->strings[0].len+1);
...
//init segmentset of active regions
      ...
      segmentset_t *set = (segmentset_t *) ALLOCMEMORY(NULL, NULL, segmentset_t, 1);
      initSegmentSet(set);
//first region
      bedln = readcsvlines(NULL, bedfi, '\t', 1, &bedcsv);
      set->nextchr = (char *) ALLOCMEMORY(NULL, NULL, char, bedcsv[0]->strings[0].len+1);
      strcpy(set->nextchr, bedcsv[0]->strings[0].str);
      set->nextstart = atoi(bedcsv[0]->strings[1].str)+1;

      set->chr = (char *) ALLOCMEMORY(NULL, NULL, char, bedcsv[0]->strings[0].len+1);
...
//add new segments
       ...
       //           fprintf(stdout,"while %d\t%d\t%d\n",bedln, pos>=set->nextstart ,(strcmp(set->chr,set->nextchr) == 0));
          while(bedln && pos>=set->nextstart && (!set->nextchr  || !set->chr || (strcmp(set->chr,set->nextchr) == 0)) ) {
              ...
              initSegment(tmp);
              tmp->chr = (char *) ALLOCMEMORY(NULL, NULL, char, bedcsv[0]->strings[0].len+1);
...
//                fprintf(stderr,"#Adding CpG %s:%s to region %s:%d-%d\n",csv[0]->strings[0].str,csv[0]->strings[1].str,seg->chr,seg->start,seg->stop);
                seg->pos = (int *) ALLOCMEMORY(NULL, seg->pos, int,    seg->n+1); //index
                seg->value = (double **) ALLOCMEMORY(NULL, seg->value, double*, seg->n+1); //cpgs

                seg->pos[seg->n] = atoi(csv[0]->strings[1].str);
                seg->value[seg->n] = (double *) ALLOCMEMORY(NULL, NULL, double, csv[0]->noofstrings);
                for(k=2; k < csv[0]->noofstrings; k++) {
...
//check missing numbers
  ...
  //        fprintf(stderr, "starting thread %d\n", i);
      ...
      chr = (char **) ALLOCMEMORY(NULL, chr, char*,   j+1); //chr
      pos = (int *) ALLOCMEMORY(NULL, pos, int,    j+1); //index
      val = (double **) ALLOCMEMORY(NULL, val, double*, j+1); //cpgs

      chr[j] = x;
      pos[j] = y;
      val[j] = (double *) ALLOCMEMORY(NULL, NULL, double, csv[0]->noofstrings);
...
//        val[j][k-2] = atof(csv[0]->strings[k].str);
  ...
  for(int i=0;i<nfo.outputList->i;i++){
    FREEMEMORY(NULL, nfo.outputList->segment_out_st[i].chr);
  }
  FREEMEMORY(NULL, nfo.outputList->segment_out_st);
