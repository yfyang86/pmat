...
void multiple_testing_correction(list_out *list, int run_mode, int mtc_method){
  ...
  /* extract p-values */
  p = (double *) ALLOCMEMORY(NULL, NULL, double, list->i);
  for(i=0; i<list->i; i++){
    if (run_mode == 1 || run_mode == 2) {
      p[i] = list->segment_out_st[i].p;
    }
    else if (run_mode == 3) {
      p[i] = list->segment_out_st[i].mwu;
    }
  }
...
double * bonferroni_correction(double *p, int n, int num_tests){
  ...
  double *q = (double *) ALLOCMEMORY(NULL, NULL, double, n);
  for (int i=0; i<n; i++){
...
double * benjamini_hochberg_correction(double *p, int n, int num_tests){
  ...
  q = (double *) ALLOCMEMORY(NULL, NULL, double, n);
