...
stringset_t* tokensToStringset(void *space, char* delim, char* toTokens, 
	...
	set = (stringset_t *) ALLOCMEMORY(space, NULL, stringset_t, 1);
	set->noofstrings = 0;
	set->strings = NULL;
	...
	buffer = (char*) ALLOCMEMORY(space, NULL, char, len+1);
	if (buffer == NULL) {
		fprintf(stderr, "copy tokenstring %s to buffer failed.\n", toTokens);
		exit(-1);	
	}
	buffer = (char *) memcpy(buffer, toTokens, len+1);
        buffer[len] = 0;
	...
	while(token != NULL) {
			...
			toklen = strlen(token);
			set->noofstrings++;
			set->strings = (string_t *) ALLOCMEMORY(space, set->strings, string_t, set->noofstrings);
			set->strings[set->noofstrings-1].str = (char*) ALLOCMEMORY(space, NULL, char, toklen+1);
			set->strings[set->noofstrings-1].str = (char*) memcpy(set->strings[set->noofstrings-1].str, token, toklen);
