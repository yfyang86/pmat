...
getNiceSVNVersion(const char *version) {
  ...
  char *p, *src;
  const char *subs[] = { "$Rev: ","$Date: ", " $" };

  src = (char *) calloc(strlen(version)+1, sizeof(char));
  strcpy(src, version);
  for(i=0; i < 3; i++) {
...
manopt_usage(manopt_optionset *set) {
  ...
  synopsis = (char *) malloc(sizeof(char)*MANOPT_MAXSYNOPSIS);
  call = (char *) malloc(sizeof(char)*MANOPT_MAXSYNOPSIS);
  call[0] = '\0',
  synopsis[0] = '\0' ;

  arg = (char **) malloc(sizeof(char*)*set->noofopts);
  msg = (char **) malloc(sizeof(char*)*set->noofopts);

  for(i=0;i<set->noofopts;i++) {
    arg[i] = (char *) malloc(sizeof(char)*MANOPT_MAXSYNOPSIS);
    arg[i][0] = 0;
    msg[i] = (char *) malloc(sizeof(char)*MANOPT_MAXSYNOPSIS);
    msg[i][0] = 0;
  }
  ...
  fill = (char *) realloc(fill, calllen*sizeof(char));
  for(i=0; i < calllen; i++) {
    fill[i]=' ';
  }
  ...
  fill = (char *) realloc(fill, maxarglen*sizeof(char));
...
manopt_parse_commandline(manopt_argset* argset, int argc, char **argv) {
  ...
  for (i=0; i < argc; i++) {
    ...
    if(argv[i][0] == '-' && (argv[i][1] < 48 || argv[i][1] > 57)) {
      offset = (argv[i][1] == '-') ? 1 : 0;
      arg = (manopt_arg *) realloc(arg, sizeof(manopt_arg)*(cnt+1));
      ...
      arg[cnt-1].values = (char **)realloc(arg[cnt-1].values,
...
manopt_blockseparator(manopt_optionset *set, char *blockname) {
  ...
  set->opts = (manopt_option *) realloc(set->opts, sizeof(manopt_option)*(set->noofopts+1));
...
manopt(manopt_optionset *set,
  ...
  set->opts = (manopt_option *) realloc(set->opts, sizeof(manopt_option)*(set->noofopts+1));
  manopt_initoption(&set->opts[set->noofopts]);
  set->opts[set->noofopts].argdesc = (char*) argdesc;
  set->opts[set->noofopts].shortopt = shortopt;
  set->opts[set->noofopts].longopt = (char*) longopt;
  set->opts[set->noofopts].helpmsg = (char*) helpmsg;
  ...
  if(reg_var) {
    set->opts[i].defaultval = (char *) malloc(sizeof(char)*MANOPT_MAXSYNOPSIS);
...
manopt_unflag(manopt_argset *argset,
  ...
  argset->args[0].values = (char **) realloc(argset->args[0].values,
  ...
  argset->args[arg].values = (char **) realloc(argset->args[arg].values,
      size*sizeof(char*));
...
manopt_getopts(manopt_optionset* set, int argc, char **argv) {
          ...
          if (set->opts[i].set) {
            ...
            if(set->opts[i].reg_var) {
              switch(set->opts[i].type) {
                case FLAG:
                  ucharval = (unsigned char *) set->opts[i].reg_var;
                  *ucharval = 1;
                  break;
                case REQCHAROPT:
                case CHAROPT:
  ...
  argset.args = (manopt_arg *) realloc(argset.args, sizeof(manopt_arg));
