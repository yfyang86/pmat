...
double *rowdel(void *space, double *a, Uint m, Uint n, Uint d)
{
...
double *cov(void *space, double *x, double *y, Uint m)
{
    double *c,
        xm,
        ym;
 ...
 *  - https://arxiv.org/pdf/2005.10449.pdf
 */
...
double mannwhitney(double *a, Uint m, double *b, Uint n, int *nonZeroPairs, bool useBartlett, double * samParam)
{
    ...
    if (samParam[5] < 0){
        ans = mannwhitney(a, m, b, n, nonZeroPairs);

    }else{

        ans = foldednomalpvalue(a, m, b, n, false, samParam, 0.00001, true, useBartlett);
 ...
 * The above link includes the wrong C code (`gsl_ran_gamma` LINE 8):
 *    return gsl_ran_gamma (r, 1.0 + a, b) * pow (u, 1.0 / a);
 * should be:
 *    return gsl_ran_gamma (r, 1.0 + a, b) * pow (u, 1.0 / b);
 *
 */
...
{
    /*thread safe: 
  * TODO: Avoid thread_local static problem
  * TEST PATH: ${PRJ}/tmp
  * TEST COMILE: g++ -o uat_test01_RND_2th -Wall -std=c++17 test01_RND_2th.cpp -lpthread
  * TEST CMD: ./uat_test01_RND_2th | grep "Mean" | awk '{print $2}' 
  */
    using params = std::pair<double, double>;
    using map_t = std::map<params, std::gamma_distribution<>>;
    static thread_local map_t range_map;
    auto i = range_map.find(params(alpha, beta));
    if (i == std::end(range_map))
    {
        i = range_map.emplace(std::make_pair(alpha, beta), std::gamma_distribution<>{alpha, beta}).first;
    }
    return i->second(prng_engine());
    /*
  std::random_device _priv_rd;
  std::mt19937  _priv_gen(_priv_rd);
 ...
 * @param alpha: double 
 * @param beta: double 
 * @return double: a random Gamma real number
 */
double randgam(double alpha, double beta)
{
    ...
    assert(alpha > 0);
    double x = st1_randgam(alpha, beta);

#endif // MARKER_st1_randgam

#ifndef MARKER_st1_randgam
    ...
    { // ? > or >=
        ...
        while (1)
        {
            ...
            if (z1 > -1.0 / c)
            {
                ...
                u = uniform_rand();
                if (log(u) < 0.5 * (z1 * z1) + d - d * v + d * log(v))
                    break;
            }
        }
        x = d * v / beta;
 ...
 * @return p-value (0 is error)
 */
double foldednomalpvalue(double *a, int m, double *b, int n, bool logit, double * samParam, double logittuning, bool nonZero, bool useBartlette)
{
    if (m != n)
    {
        return 0.;
    ...
    {
        if (useBartlette){
            re = LRFstatNative(x, i, corr_v[5], useBartlette, samParam);
        }else{
            re = LRFstatNative(x, i);
        }
